...
impl FileStructure {
    ...
    pub fn load_shadow_from<P: AsRef<Path>>(&self, path: P) -> Option<FileStructure> {
        match self {
            FileStructure::Dir(name, content) => {
                let shadowed = path.as_ref().join(name);
                if shadowed.exists() {
                    let shadowed_content = content
                        .iter()
                        .map(|fs| fs.load_shadow_from(shadowed.as_path()))
                        .filter(|fs| fs.is_some())
                        .map(|fs| fs.unwrap())
                        .collect();
                    Some(FileStructure::Dir(name.clone(), shadowed_content))
                } else {
                    None
                }
            },
            FileStructure::File(name, code) => {
                ...
                if shadowed.exists() {
                    let mut file = File::open(path)
                        .expect(format!("Could not open {}", shadowed.display()).as_str());
...
impl CodePage {
    const DESCRIPTION: &'static str = " THIS FILE IS GENERATED BY PAKKEN. EVERY FRAGMENT MARKED \
                                       WITH @GENRATED WILL BE OVERRIDEN ON THE NEXT GENERATION. \
    ...
    pub fn from(comment_string: &'static str, code: &str) -> CodePage {
        ...
        let gen_end = Self::end_generated_str(comment_string);
        let gen_end_str = gen_end.as_str();
        let mut next = code.find(gen_start_str);

        while next.is_some() {
            let next_code = code.split_at(next.expect("Should have next.")).1;
            ...
            let fragment_code = new_code.split_at(end_code).0;
            let fragment_id = comment.split_at(gen_start_str.len()).1.trim();
            let fragment = CodeFragment::Generated(GeneratedCode {
                code: fragment_id.to_string(),
                id: fragment_code.to_string(),
            });
            codepage.add(fragment);
            // TODO handle other code fragments that are not generated (anymore)
            next = next_code.find(gen_start_str);
        }
    ...
    fn end_generated_str(comment_string: &str) -> String {
        let mut buffer = Buffer::default();
        buffer += comment_string;
        buffer += " ";
        ...
        for fragment in &self.fragments {
            ...
            match fragment {
                CodeFragment::Generated(generated) => {
                    ...
                    buffer.new_line();
                    buffer += Self::end_generated_str(self.comment_string).as_str();
                },
